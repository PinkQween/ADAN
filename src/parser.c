...
ASTNode *parse_program(Parser *parser)
{
	...
	if (!expect(parser, TOKEN_PROGRAM, PARSER_EXPECTED, "keyword 'program'", parser->current_token.text))
		return NULL;
	if (!expect(parser, TOKEN_TYPE_DECL, PARSER_EXPECTED, "type declarator '::' when defining a type", parser->current_token.text))
		return NULL;
...
ASTNode *parse_array_literal(Parser *parser)
{
	...
	while (parser->current_token.type != TOKEN_RBRACKET && parser->current_token.type != TOKEN_EOF)
	{
		...
		if (!tmp)
		{
			set_error(parser, PARSER_ALLOCATION_FAILURE, "array elements");
			free_ast(element);
			free_ast(array_node);
			return NULL;
		}
...
ASTNode *parse_assignment(Parser *parser)
{
	...
	if (next == TOKEN_ADD_IMMEDIATE || next == TOKEN_SUB_IMMEDIATE || next == TOKEN_MUL_IMMEDIATE || next == TOKEN_DIV_IMMEDIATE || next == TOKEN_MOD_IMMEDIATE)
	{
		...
		{
		case TOKEN_ADD_IMMEDIATE:
			bin_token.type = TOKEN_PLUS;
			bin_token.text = "+";
			break;
		case TOKEN_SUB_IMMEDIATE:
			bin_token.type = TOKEN_MINUS;
			bin_token.text = "-";
			break;
		case TOKEN_MUL_IMMEDIATE:
			bin_token.type = TOKEN_ASTERISK;
			bin_token.text = "*";
			break;
		case TOKEN_DIV_IMMEDIATE:
			bin_token.type = TOKEN_SLASH;
			bin_token.text = "/";
			break;
		case TOKEN_MOD_IMMEDIATE:
			bin_token.type = TOKEN_PERCENT;
			bin_token.text = "%";
			break;
		default:
			free_ast(identifier);
...
ASTNode *parse_binary(Parser *parser)
{
	...
	while (1)
	{
		...
		/* Debug: print left node before precedence check */
		if (left == NULL) {
			fprintf(stderr, "DEBUG: parse_binary: left is NULL\n");
		} else {
			fprintf(stderr, "DEBUG: parse_binary: left=%p left->type=%d left->token.type=%d\n", (void*)left, left->type, left->token.type);
		}
		int left_prec = get_node_precedence(left);
		if (!right_assoc && precedence < left_prec)
			break;
		if (right_assoc && precedence <= left_prec)
			break;
...
ASTNode *parse_if_statement(Parser *parser)
{
	...
	if (!expect(parser, TOKEN_IF, PARSER_EXPECTED, "'if'", parser->current_token.text))
		return NULL;
	if (!expect(parser, TOKEN_LPAREN, PARSER_EXPECTED, "'('", parser->current_token.text))
		return NULL;
...
ASTNode *parse_while_statement(Parser *parser)
{
	...
	if (!expect(parser, TOKEN_WHILE, PARSER_EXPECTED, "'while'", parser->current_token.text))
		return NULL;
	if (!expect(parser, TOKEN_LPAREN, PARSER_EXPECTED, "'('", parser->current_token.text))
		return NULL;
...
{
	if (!expect(parser, TOKEN_FOR, PARSER_EXPECTED, "'for'", parser->current_token.text))
		return NULL;
	if (!expect(parser, TOKEN_LPAREN, PARSER_EXPECTED, "'('", parser->current_token.text))
		return NULL;
	...
	Token id_token = parser->current_token;
	if (id_token.text)
		id_token.text = strdup(id_token.text);
	...
	if (inc_id_token.text)
		inc_id_token.text = strdup(inc_id_token.text);
	...
	if (inc_op_token.text)
		inc_op_token.text = strdup(inc_op_token.text);
...
ASTNode *parse_include_statement(Parser *parser)
{
	...
	if (!expect(parser, TOKEN_INCLUDE, PARSER_EXPECTED, "'include'", parser->current_token.text))
		return NULL;
	ASTNode *publisher = parse_include_name(parser);
	if (!publisher)
		return NULL;
	if (!expect(parser, TOKEN_PERIOD, PARSER_EXPECTED, "'.'", parser->current_token.text))
		return NULL;
	...
	case TOKEN_LPAREN:
	{
			...
			parser->current_token.type == TOKEN_CHAR)
		{
			...
			if (type_token.text)
				type_token.text = strdup(type_token.text);
			match(parser, parser->current_token.type);
			if (!expect(parser, TOKEN_RPAREN, PARSER_EXPECTED, "')' after cast type", parser->current_token.text))
				return NULL;
